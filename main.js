...
$(document).ready(() => {
  ...
  const checkFormValidity = () => {
    ...
    if (validateEmail(email) && validatePassword(pw)) $("#loginBtn").prop("disabled", false);
    else $("#loginBtn").prop("disabled", true);
  };

  $("#email").on("keyup blur", function(){
    const email = $(this).val().trim();
    if (!email || !validateEmail(email)) $("#emailError").text("Please enter a valid Northeastern email");
  ...
  $("#password").on("keyup blur", function(){
    ...
    else if (pw.length < 8) $("#passwordError").text("Password must be at least 8 characters");
  ...
  $("#loginForm").on("submit", function(e){
    ...
    const sessionData = {
      username, email, timestamp: new Date().toISOString(), isLoggedIn: true
    };
  ...
  if (window.location.pathname.endsWith("calculator.html")) {
    const session = JSON.parse(sessionStorage.getItem("userSession") || localStorage.getItem("userSession") || "null");
    ...
    $(".op-btn").click(function(){
      ...
      if (isNaN(n1)){ $("#num1Error").text("Please enter a valid number"); valid = false; }
      else $("#num1Error").text("");

      if (isNaN(n2)){ $("#num2Error").text("Please enter a valid number"); valid = false; }
      else $("#num2Error").text("");
